...
category: Swift
excerpt: >
  Le sujet de cette semaine est le protocol `Hashable`
...
et ce dernier impose la même contrainte à son type générique `Key`.

Dans des versions précédentes du language, une bonne dose
...
Considérons le type `Color` suivant, qui représente une
...
entre toutes leurs propriétés d'un objet, et n'allaient pas
plus loin.
...
la famille de fonctions pseudo-aléatoires [SipHash](https://en.wikipedia.org/wiki/SipHash), plus précisément [SipHash-1-3 and SipHash-2-4](https://github.com/apple/swift/blob/master/stdlib/public/core/SipHash.swift), avec respectivement 1 à 2 cycles
de hachage par block de message et 3 à 4 cycles de finalisation.
